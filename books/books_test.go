...
const (
	bookKey              = "filename"
	contentKey           = "content"
	bookTitleNonExistent = "La Odisea"
...
func (bf *BookFeature) theBookContentIsReturned(ctx context.Context) error {
	...
	//transform []byte content to string and you can do comparison between two strings.
...
func (bf *BookFeature) theBookContentIsUpdated() error {
	...
	//change pointer backwards from end of file
	_, err := bf.client.book.Seek(int64(-len(content)), 2)
	if err != nil {
		return err
	}
	_, err = bf.client.book.Read(buf)
	if err != nil {
		return err
	}
	contentRead := string(buf)
	fmt.Println("CONTENT READ FROM BUFFER: " + contentRead)
	if contentRead != content {
		fmt.Errorf("Read %v from buffer but expected %s", string(buf), content)
	}
	return nil
}
...
func (bf *BookFeature) theBookIsCreatedAndUpdated(ctx context.Context) error {
	...
	bookTitle, ok := ctx.Value(bookTitleNonExistent).(string)
	if !ok {
...
func (bf *BookFeature) iDeleteTheBook() error {
	//get book title from filename. It contains the relative path (booksfile/laodisea.txt) so we have to clean up and extract only laodisea
	//split return in a string slice [0] -> booksfile and [1] -> laodisea.txt
	filenameWithExt := strings.Split(bf.client.book.Name(), "/")
	bookTitleformat := strings.Split(filenameWithExt[2], ".txt")[0]
	defer bf.closeFile()
	err := bf.client.DeleteBook(bookTitleformat)
	if err != nil {
		return err
	}
	return nil
...
func (bf *BookFeature) closeFile() {

	if err := bf.client.Close(); err != nil {
...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^the book "([^"]*)" does not exists$`, bookFeature.theBookDoesNotExist)
	...
	ctx.Step(`^I delete the book$`, bookFeature.iDeleteTheBook)
